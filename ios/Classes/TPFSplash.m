...
#define TP_SPLASH_VERBOSE_LOGGING NO
#define TPSplashDebugLog(fmt, ...) do { if (TP_SPLASH_VERBOSE_LOGGING) { NSLog((fmt), ##__VA_ARGS__); } } while (0)
...
- (void)logEvent:(NSString *)event adInfo:(NSDictionary *)adInfo error:(NSError *)error
{
    TPSplashDebugLog(@"[tradplus][ios][splash] event=%@ adUnitID=%@ adInfo=%@ error=%@", event, self.splash.unitID, adInfo, error);
}
...
- (void)tpSplashAdLoaded:(NSDictionary *)adInfo
{
    ...
    [self logEvent:eventNam adInfo:adInfo error:nil];
...
- (void)tpSplashAdLoadFailWithError:(NSError *)error
{
    ...
    [self logEvent:eventNam adInfo:nil error:error];
...
- (void)tpSplashAdShow:(NSDictionary *)adInfo didFailWithError:(NSError *)error
{
    ...
    [self logEvent:eventNam adInfo:adInfo error:error];
...
- (void)tpSplashAdAllLoaded:(BOOL)success
{
    ...
    TPSplashDebugLog(@"[tradplus][ios][splash] event=%@ adUnitID=%@ success=%@", eventNam, self.splash.unitID, @(success));
