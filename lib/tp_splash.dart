...
const bool _tpSplashVerboseLoggingEnabled = false;

void _tpSplashLog(String message) {
  if (_tpSplashVerboseLoggingEnabled) {
    debugPrint(message);
  }
}
...
class TPSplash {
  ...
  setSplashListener(TPSplashAdListener listener, {String adUnitId = ""}) {
    if (adUnitId.isNotEmpty) {
      _tpSplashLog('[tradplus][splash] set listener for adUnitId=$adUnitId');
      TPListenerManager.splashAdListenerMap[adUnitId] = listener;
    } else {
      _tpSplashLog('[tradplus][splash] set global splash listener');
